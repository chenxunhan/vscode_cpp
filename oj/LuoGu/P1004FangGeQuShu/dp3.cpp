...
/*dpMaxValue[step][x1][x2]: 当Rob1和Rob2各走了step步后
Rob1 走到第 x1行第 step+2-x1列 而 Rob2走到第 x2行第 step+2-x2列 可收集的最大样本价值,
dpMaxValue[2 * tableLength][tableLength][tableLength]就是最终的答案*/
            ...
            for (int x2 = 1, x2Lim = min(step - 1, tableLength); x2 <= x2Lim; ++x2) {
                ...
                dpMaxValue[step][x1][x2] =
                    max( //记录着所有的四个上次状态中收集到的最大样本价值, 代表从该状态转移而来
                        max(
                            dpMaxValue[step - 1][x1 - 1][x2],      //Rob1从左边向右走来, Rob2从上边向下走来
                            dpMaxValue[step - 1][x1 - 1][x2 - 1]), //Rob1从左边向右走来, Rob2从上边向下走来
                        max(
                            dpMaxValue[step - 1][x1][x2],       //Rob1从上边向下走来, Rob2从左边向右走来
                            dpMaxValue[step - 1][x1][x2 - 1])); //Rob1从上边向下走来, Rob2从上边向下走来
                dpMaxValue[step][x1][x2] += table[x1][step - x1];
                /*若Rob1和Rob2在此次状态走到了不同的2个格子, 则table[x1][step - x1] != table[x2][step - x2],
                dpMaxValue要加上table[x2][step - x2]*/
                if (x1 != x2) {
